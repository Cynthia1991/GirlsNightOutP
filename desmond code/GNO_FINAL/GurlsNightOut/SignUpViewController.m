...
- (IBAction)textfieldReturn:(id)sender{
    ...
    self.myScollView.frame=CGRectMake(0, 0, 320, 460);
}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    self.myScollView.frame=CGRectMake(0, 0, 320, 244);
    self.myScollView.contentSize=CGSizeMake(320, 460);
...
- (IBAction)SignUpAction:(id)sender {
    if ([tfPassword.text isEqualToString:tfConfirmPW.text]) {
        ...
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Miss information" message:@"Please enter the your name, email and password" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
        }
    }
    else
