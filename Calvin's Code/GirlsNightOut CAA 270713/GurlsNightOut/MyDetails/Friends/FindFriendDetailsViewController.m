...
- (IBAction)addFirendAction:(id)sender {
    if ([[appdelegate.myDetailsManager.userInfoDic objectForKey:@"userDBID"] intValue]!=[self.friendUserDBID intValue]) {
        Kumulos *k=[[Kumulos alloc] init];
        [k setDelegate:self];
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"You cannot add yourself to friend list" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    }
